...
########################################
######################################## INITIAL DESIGN OF EXPERIMENTS & surrogate models
######################################## 
...
for(i in 1:iterations)
{
  ...
  g <- function(utilde) return(aggregate_u_tp1(utilde,x_tplus1,Opt_,rep=5,min_feas_init$min,model_O,
                                               alea,m,d,listCon,seuil,sign)) 
  registerDoFuture()
  plan(multisession, workers = max(detectCores()-1, 1))
  optu = future_apply(Opt,1,g)
  u_tplus1 <- Opt_[which.min(optu),(d+1):(d+m)]
  
  ##### Update metamodel
  
  point_new = cbind(matrix(x_tplus1,1,d),matrix(u_tplus1,1,m))
  design_init = rbind(design_init, point_new)
  response_f = c(response_f, Objective(point_new))
  response_g_1 = c(response_g_1, Constraint1(point_new)) 
  response_g_2 = c(response_g_2, Constraint2(point_new)) 
  ...
  listreponses <- list(O=response_f,C1=response_g_1,
                       C2=response_g_2)
  
  resultat <- list(modelObj=model_O,model_C=listCon,responses=listreponses,
                   design=design_init,feasminf=hist_feasmin,Optdesign=hist_feasopt)
}
