...
d <- 2 #2 # Dimension of X
m <- 2 #2 # Dimension of U        
n_g = 2 # NUmber of constraints

Nsim <- 100 #1000 #N trajectoires pour le calcul de P(C < 0) 
seuil <- 0. #0 #seuil
alpha <- 0.05 #proba
beta <- 1 - alpha
quantizer <- 10

U_MC <- 200 #300 #CRN pour U
N_xOpt <- 2000 #200
N_uOpt <- 300 #200
...
for(rep in 1:repetitions)
{
  ...
  df1 = data.frame(design_init_g1,response_g_1)
  df2 = data.frame(design_init_g2,response_g_2)
  df = list(df1,df2)
